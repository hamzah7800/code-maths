window.addEventListener('DOMContentLoaded', function () {
  ...
  const createScene = function () {
    ...
    const player = BABYLON.MeshBuilder.CreateCapsule("player", { radius: 1, height: 3 }, scene);
    player.position = new BABYLON.Vector3(0, 2, 0); // Ensure it spawns above ground
    ...
    setTimeout(() => { // Delay to ensure physics applies correctly
      player.physicsImpostor = new BABYLON.PhysicsImpostor(
        player, BABYLON.PhysicsImpostor.CapsuleImpostor, { mass: 1, restitution: 0.1 }, scene
      );
    }, 100);
    ...
    camera.lockedTarget = player; // Camera follows the player
    camera.radius = 10;
    ...
    // Multiplayer configuration using Socket.io
    const socket = io();
    socket.on('connect', () => {
      console.log('Connected to server');
      socket.emit('join', { playerId: socket.id });
    });

    socket.on('updatePlayers', players => {
      players.forEach(playerData => {
        if (!scene.getMeshByID(playerData.id)) {
          const otherPlayer = BABYLON.MeshBuilder.CreateCapsule("otherPlayer", { radius: 1, height: 3 }, scene);
          otherPlayer.position = playerData.position;
          otherPlayer.id = playerData.id;
          shadowGenerator.addShadowCaster(otherPlayer);
        } else {
          const otherPlayer = scene.getMeshByID(playerData.id);
          otherPlayer.position = playerData.position;
        }
      });
    });
